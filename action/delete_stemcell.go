...
func (a CPI) DeleteStemcell(cid apiv1.StemcellCID) error {
	...
	a.logger.Debug("delete_stemcell", "deleting template %s (%s)", template.Id, cid.AsString())
	...
	a.logger.Debug("delete_stemcell", "delete_stemcell succes : template %s (%s)", template.Id, cid.AsString())
	return nil
}
