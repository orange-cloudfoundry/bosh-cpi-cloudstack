...
func (a CPI) CreateVM(
	...
	args := CreateArgs{agentID, stemcellCID.AsString(), cloudProps, networks, associatedDiskCIDs, env}
...
func (a CPI) CreateBase(p CreateArgs, isV2 bool) (apiv1.VMCID, apiv1.Networks, error) {
	...
	serviceOffering, err := a.serviceOfferingFindByName(resProps.ComputeOffering)
	if err != nil {
		return apiv1.VMCID{}, apiv1.Networks{}, err
	}

	template, err := a.templateFindByName(p.stemcellName, zone)
	if err != nil {
		return apiv1.VMCID{}, apiv1.Networks{}, err
	}

	if err = a.validateVMNetworks(p.networks); err != nil {
		return apiv1.VMCID{}, apiv1.Networks{}, err
	}
	...
	deplParams := a.client.VirtualMachine.NewDeployVirtualMachineParams(serviceOffering.Id, template.Id, zone.Id)
	...
	networkDefault, networkMaps, err := a.generateNetworksMap(p.networks, zone.Id)
	if err != nil {
...
// addRouteToNetworks - configure networks according to route vm_extensions
func (a CPI) addRouteToNetworks(routes VMExtRouteMap, boshNetworks apiv1.Networks) error {
	for networkName, routeList := range routes {
		...
		for _, route := range routeList {
			...
			if err != nil {
				return bosherr.WrapErrorf(err, "invalid vm_extension: malformed CIDR '%s'", route)
			}
			if route.Gateway == "" {
...
func (a CPI) attachEphemeralDisk(vmCID apiv1.VMCID, diskCid apiv1.DiskCID) error {
	return a.retryable(5 * time.Minute,
		func() error {
...
func (a CPI) createVip(network apiv1.Network, vmId, zoneID string, defNetwork *cloudstack.Network) error {
	...
	return a.retryable(16*time.Minute,
		func() error {
...
func (a CPI) validateVMNetworks(networks apiv1.Networks) error {
	var (
		nbManual  int
		nbDynamic int
		nbVip     int
